...
def lambda_handler(event, context):
    ...
    # --- ここから追加：output.zip を自動展開して S3 に保存 ---
    # 環境変数（CDK 側で設定）
    UNZIP_PREFIX = os.environ.get("UNZIP_PREFIX", "unzipped/")
    DELETE_ZIP_AFTER_UNZIP = os.environ.get("DELETE_ZIP_AFTER_UNZIP", "false").lower() == "true"
    S3_SSE = os.environ.get("S3_SSE", "AES256")  # SSE-S3

    if targetObject.lower().endswith(".zip"):
        # 例: <configId>/<YYYYMMDD-hhmmss>/output.zip
        base_prefix = f"{configId}/{currentDate}/"
        tmp_zip = "/tmp/output.zip"

        # /tmp にダウンロード
        s3.download_file(targetBucket, targetObject, tmp_zip)

        # 展開して 1 ファイルずつ PutObject（ContentType と SSE を付与）
        with zipfile.ZipFile(tmp_zip, "r") as zf:
            for member in zf.infolist():
                ...
                s3.put_object(
                    ...
                    ContentType=_content_type(out_key),
                )

        # オプション：zip 自体を削除（保存量削減）
        if DELETE_ZIP_AFTER_UNZIP:
            s3.delete_object(Bucket=targetBucket, Key=targetObject)
    # --- 追加ここまで ---
